                ...
                <div class="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-6">
                    <div class="flex flex-col rounded-lg bg-white sm:flex-row">
                        <img class="m-2 h-24 w-28 rounded-md border object-cover object-center"
                            src="{{ asset('storage/') . '/' . $reservedBooks->buku->front_book_cover }}" alt="" />
                    ...
                    <div class="relative mt-4">
                        ...
                        <div class="flex justify-center">
                            @if ($reservedBooks->status_peminjaman == 'reserved')
                                <button class="mt-4 w-2/4 rounded-md bg-emerald-900 px-6 py-3 font-medium text-white"
                                    disabled>{{ ucwords($reservedBooks->status_peminjaman) }}</button>
                            @elseif($reservedBooks->status_peminjaman == 'borrowed')
                                <button class="mt-4 w-2/4 rounded-md bg-sky-600 px-6 py-3 font-medium text-white"
                                    disabled>{{ ucwords($reservedBooks->status_peminjaman) }}</button>
                            @elseif($reservedBooks->status_peminjaman == 'overdue')
                                <button class="mt-4 w-2/4 rounded-md bg-rose-950 px-6 py-3 font-medium text-white"
                                    disabled>{{ ucwords($reservedBooks->status_peminjaman) }}</button>
                            @endif
                        </div>
